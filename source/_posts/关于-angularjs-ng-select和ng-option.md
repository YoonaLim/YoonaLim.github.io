...
由于之前已经有类似接口可以给我提供数据，返回的数据接口如![](关于-angularjs-ng-select和ng-option/数据结构.png)
...
angular告诉我们，直接点出属性即可：`x.text for x in object.property`

接下来就涉及到双向绑定。这里已经指定了ng-model，获取选中的值，也非常方便了。由于代码是我修改完发出来的，所以上文中可以查看代码图片，就不过多解释了。

之后在js中用angular.copy将object返显出来，就达到目的啦。
